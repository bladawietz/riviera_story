...
#define SpriteCollision
#define CharCollision
#define BeerCheck
...
.const sprite_memory    =   $2400 
.const char_memory      =   $2000
.const zmienne          =   $3000


.var sprite_data=LoadBinary("riviera_sprites_0_3.bin")
//.var sprite_data_attrib=LoadBinary("riviera_sprites_02_attr.bin")

.var level_1=LoadBinary("riviera_char_map_0_3.bin")
.var level_1_char=LoadBinary("riviera_char_0_3.bin")
.var level_1_char_color=LoadBinary("riviera_char_atrib_0_3.bin")
...
nie_prawo:
    ...
    and #%00000001 //góra
    bne nie_pijemy
    lda jump        //gdy skok <>0 to nie pijemy
    bne nie_pijemy
    lda zeskok
    bne nie_pijemy  //podczas zeskoku nie pijemy
    
    //sprawdzamy czy jest butelka w okolicy ;)
    ldx #$00
    //ldy #$00
butelka_loop:
    lda chck+2,x
    ...
    cpx #$03
    bne butelka_loop
    jmp nie_pijemy // nie było nigdzie butelczyny
...
nie_pijemy:
    ...
    //zeskok z platformy
    lda JOY_2
    and #%00010010 //5 i 2 bit dół +fire
    bne nie_dol_fire
    lda chck+$0a
    ...
    lda zeskok
    cmp #$00
    bne nie_fire    //jest w trakcie zeskoku
    lda ludzik_y
    cmp #$d0
    bcs nie_dol_fire //jestesmy na samym dole?
    lda #$01
    sta zeskok
    lda #$00
    sta zeskok_indeks
...
joy_fire_tak:
 ...
 //f4 38 20 38
    ...
    lda #$04
    sta piwo
    lda piwo_delay_start
    sta piwo_delay
    lda #$05
    sta piwo_count

    //resetuj zmienną kaputt
    lda #$00
    sta kaputt
...
pijemy_piwo:
    dec piwo_delay
    bne pomin_piwo
    //jesli delay jest 0 to wpisz = piwo_delay_start
    lda piwo_delay_start
    sta piwo_delay
    lda piwo
    clc
    adc piwo_count
    cmp #$09    // jeśli piwo=4 i piwo_count=5 to już nic nie dodawaj!!!!
    beq nie_dodawaj_piwo
    inc piwo
nie_dodawaj_piwo:
    lda piwo
    cmp #$05
    bne nie_dodawaj_piwo_count
     //czy pełny pasek piwa???
    lda piwo_count
    cmp #$05
    beq pomin_piwo
    //pasek nie jest pełny
    lda #$01
    sta piwo 
    inc piwo_count
 nie_dodawaj_piwo_count:
    jsr piwna_obsluga
    rts
...
obsluga_animacji:
// animacja ognia i butelki

    dec animacja_tlo_indeks
    bne po_animacji
    lda animacja_tlo_speed
    sta animacja_tlo_indeks
    lda animacja_tlo // negacja nr. klatki
    eor #$01
    sta animacja_tlo
    //przepisujemy ogień
    ldx #$00
petla_przepisania:
    lda animacja_tlo
    bne druga_klatka
    lda $2010+$2c8,x  // ogień dół
    sta $2010,x
    lda $2010+$2d8,x  // ogień góra
    sta $2018,x
    lda $2010+$2e8,x  // butelka góra
    sta $2020,x
    lda $2010+$2f8,x  // butelka dół
    sta $2028,x
  
    jmp koniec_przepisywania
druga_klatka:
    lda $2010+$2d0,x  // ogień dół
    sta $2010,x
    lda $2010+$2e0,x  // ogień góra
    sta $2018,x
    lda $2010+$2f0,x  // butelka góra
    sta $2020,x
    lda $2010+$300,x  // butelka dół
    sta $2028,x
...
pijemy:
    .byte $00 // 0 -nic, 1 - w trakcie picia
piwo:
    .byte $04
piwo_delay:
    .byte $4a
